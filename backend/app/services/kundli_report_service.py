...
def _extract_api_divisional(full_raw: Optional[Dict], chart_code: str) -> Optional[Dict]:
    """Prefer the API's own divisional-chart data when present — it's
    already been generated by the provider and may reflect house-system
    nuances beyond the simple classical formula."""
    if not full_raw:
        return None
    try:
...
def _compute_navamsha_chart(chart: Dict, full_raw: Optional[Dict]) -> Optional[Dict]:
    """Fallback used when the API doesn't provide D9 directly — computes
    it ourselves from each planet's sign + degree via the fixed classical
    formula above. Requires the Ascendant's degree to be resolvable;
    individual planets missing degree data are simply omitted from the
    resulting chart rather than guessed."""
...
def _get_d9_chart(chart: Dict, full_raw: Optional[Dict]) -> Optional[Dict]:
    return _extract_api_divisional(full_raw, "D9") or _compute_navamsha_chart(chart, full_raw)
...
def generate_kundli_report_pdf(session_id: str, language: str, on_progress=None) -> bytes:
    ...
    # ================= PAGE 3 — D9 NAVAMSHA CHART (only if computable) =================
